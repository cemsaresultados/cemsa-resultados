...
document.getElementById('loginForm').addEventListener('submit', async function(e) {
    ...
    // 2. Validar que el captcha no esté vacío
    if (!captchaToken) {
        alert("Por favor, confirma que no eres un robot marcando la casilla de reCAPTCHA.");
        return;
    }
    ...
    try {
        const response = await fetch('/api/consultar', {
            ...
            body: JSON.stringify({ 
                ...
                usuario, 
                password,
                captchaToken // <--- Enviamos el token del reCAPTCHA al backend
            })
        });
        ...
        if (!data.success) {
            ...
            if (typeof grecaptcha !== 'undefined') grecaptcha.reset(); // Reiniciar widget si falló la validación
            return;
        }
        ...
        listaResultados.innerHTML = `
            <div style="padding: 20px; border: 1px solid var(--border-color); border-radius: 10px; background: #f8fafc; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
                    ...
                    vistaDinamica.innerHTML = `
                        ...
                        <div style="display: grid; gap: 12px; font-size: 0.9rem;">
                            <div style="display: flex; justify-content: space-between; align-items: center; background: #f8fafc; padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px;">
                                ...
                                <strong style="color: #047857;">Producción (Cloud Render)</strong>
                            </div>
                            <div style="display: flex; justify-content: space-between; align-items: center; background: #f8fafc; padding: 10px; border: 1px solid #cbd5e1; border-radius: 6px;">
                        ...
                        <form id="formSubidaPdf" style="display: grid; gap: 12px;">
                            ...
                            <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px;">
                                ...
                                <div>
                                    ...
                                    <select id="estadoExamenSubida" style="width: 100%; padding: 8px; border: 1px solid #cbd5e1; border-radius: 6px;"><option value="disponible">🟢 Disponible</option><option value="en proceso">🔵 En Proceso</option></select>
                                </div>
                                <div>
                        ...
                        <div style="overflow-x: auto;">
                            <table style="width: 100%; border-collapse: collapse; font-size: 0.9rem;">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td style="padding: 8px; border: 1px solid #cbd5e1;">Dra. Laura Jiménez</td>
                                        <td style="padding: 8px; border: 1px solid #cbd5e1;">Salud Ocupacional</td>
                                        <td style="padding: 8px; border: 1px solid #cbd5e1;">RM-88210</td>
                                        <td style="padding: 8px; border: 1px solid #cbd5e1; color: #047857; font-weight: bold;">Activo</td>
                                    </tr>
                                </tbody>
                            </table>
